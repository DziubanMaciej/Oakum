...
/// @details Sometimes it may not possible to resolve the symbol (e.g. when binary was compiled in Release configuration). For these cases the
/// user can specify #OakumInitArgs.fallbackSymbolName, which will be used instead.
/// @param[in] allocations array of allocations to resolve symbols.
/// @param[in] allocationsCount size of the @p allocations array.
/// @return #OAKUM_UNINITIALIZED, if #oakumInit has not been called.
/// @return #OAKUM_INVALID_VALUE, if @p allocations is `NULL` and @p allocationsCount is not zero.
/// @return #OAKUM_INVALID_VALUE, if @p allocations is not `NULL` and @p allocationsCount is zero.
/// @return #OAKUM_FEATURE_NOT_SUPPORTED, if the library is not tracking stack traces or symbols are not available (see #OakumCapabilities).
...
/// @details Sometimes it may not possible to resolve the source locations (e.g. when binary was compiled in Release configuration). For these cases the
...
/// @return #OAKUM_INVALID_VALUE, if @p allocations is `NULL` and @p allocationsCount is not zero.
/// @return #OAKUM_INVALID_VALUE, if @p allocations is not `NULL` and @p allocationsCount is zero.
/// @return #OAKUM_FEATURE_NOT_SUPPORTED, if the library is not tracking stack traces or source location are not available (see #OakumCapabilities).
